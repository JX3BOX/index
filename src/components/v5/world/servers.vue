<template>
    <section class="m-world-v5-servers">
        ...
        <div class="m-world-v5-servers__panel">
            <div class="u-row">
                <span class="u-label">全服平均延迟</span>
                <span class="u-latency">{{ avgHeatText }}</span>
            </div>
            <div class="u-row">
                <button class="u-server-trigger" type="button" @click="toggleSheet" :disabled="!visibleServers.length">
                    <span class="u-server-name">{{ currentServerName || "请选择服务器" }}</span>
                    <i class="u-arrow el-icon-arrow-down" :class="{ 'is-open': sheetVisible }"></i>
                </button>
                <div class="u-state" :class="stateClass">
                    <i class="u-dot"></i>
                    <span>{{ stateText }}</span>
                </div>
            </div>
        ...
        <transition name="w-world-v5-sheet-fade">
            <div v-if="sheetVisible" class="w-world-v5-sheet__mask" @click="closeSheet">
                <div class="w-world-v5-sheet" @click.stop>
                    <div class="w-world-v5-sheet__title">服务器状态</div>
                    <div class="w-world-v5-sheet__list">
                        <button
                            v-for="(item, index) in visibleServers"
                            :key="`${item.serverName}-${index}`"
                            class="w-world-v5-sheet__item"
                            :class="{
                                'is-active': item.serverName === currentServerName,
                                [getHeatState(item).class]: true,
                            }"
                            type="button"
